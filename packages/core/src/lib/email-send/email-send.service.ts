...
export class EmailSendService {
	...
	public async getInstance(): Promise<Email<any>> {
		try {
			...
			this.logger.error(`Error while retrieving default global smtp configuration: ${error}`);
	...
	public async getEmailInstance({
		...
		try {
			...
			if (await SMTPUtils.verifyTransporter(transport)) {
				...
				this.logger.error(
					`SMTP configuration is not set for this tenant / organization: [${organizationId}, ${tenantId}]`
				);
			...
			try {
				if (error instanceof NotFoundException) {
					...
					if (await SMTPUtils.verifyTransporter(transport)) {
						...
						this.logger.error(`SMTP configuration is not set for this tenant: ${organizationId}`);
				...
				if (await SMTPUtils.verifyTransporter(transport)) {
					...
					this.logger.error(
						`Error while retrieving tenant/organization smtp configuration: ${error?.message}`
					);
