import { BadRequestException, HttpStatus, Injectable, Logger as NestLogger } from '@nestjs/common';
...
export class EmailConfirmationService {
	...
	public async resendConfirmationLink(config: IAppIntegrationConfig) {
		...
		try {
			...
			this.logger.error(`Error while resending confirmation link: ${error}`);
	...
	public async decodeConfirmationToken(token: IUserTokenInput['token']): Promise<IUser> {
		...
		try {
			...
			this.logger.error(`Error while decoding confirmation token: ${error}`);
			throw new BadRequestException(error?.message);
		}
	}
	...
	public async confirmationByCode(
		...
		try {
			...
			this.logger.error(`Error while confirming email by code: ${error}`);
			throw new BadRequestException('Failed to verify email.');
		}
	}
